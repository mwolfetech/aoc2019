;;when destination is relative!
(ns sensor-boost.base
  (:gen-class)
...
;; TODO relative relative-base adjustment
...
(defn adjust-dest [state arity dest]
  ;; TODO this is not working.....needs to use the right opcode
...
(defn read-input [state]
  (let [fp (get-function-parameters state 0 1)
        ...
        val (<!! (:in state))]
      ;;(pplog state ["read-input" dest "<-" val "debug:" fp ])
      (-> state (move-pc fp)
