...
(defn make-graph [input]
  (let [graph (dag/graph)]
    (reduce (fn [seen d] (dag/depend seen (:step d) (:dependency d))) graph (pattern-seq [:step :dependency] #"([A-Z0-9]+)\)([A-Z0-99]+)(?:\r?\n)" input))))

(defn part-one []
  (let [graph (make-graph input)]
    (->> (dag/nodes graph)
         (map (comp seq #(dag/transitive-dependents graph %)))
         (remove nil?)
         (flatten)
         (count))))
...
(defn -main [& _args]
    (doall (map println [(part-one)(part-two)])))
