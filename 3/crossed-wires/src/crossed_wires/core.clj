...
(defn manhattan-range [a b]
  (cond (= a b) '(a)
        (< a b) (range (inc a) (inc b))
        :else  (range b a)))

(defn to-point-sequence
  "add origin and point sequence to segment excluding origin"
  [origin segment]
  (let [magnitude (Integer. (:magnitude segment))
        direction (:direction segment)
        [x1 y1] origin
        points  (vec
                 (case direction
                   "L" (for [x (manhattan-range x1 (- x1 magnitude))] [x y1])
                   "R" (for [x (manhattan-range x1 (+ x1 magnitude))] [x y1])
                   "U" (for [y (manhattan-range y1 (+ y1 magnitude))] [x1 y])
                   "D" (for [y (manhattan-range y1 (- y1 magnitude))] [x1 y])))
        dest (case direction
               "L" (first points)
               "R" (last points)
               "D" (first points)
...
(defn parse
  ...
  (->> input
       ...
       (map #(->> %
                  (re-seq #"(R|L|D|U)(\d+)")
                  (map (comp (partial zipmap [:direction :magnitude]) rest))))))

(defn get-point-set [segment]
  (let [find-set (comp set :points)]
    (find-set segment)))

(defn cross? [segment1 segment2]
  (seq (clojure.set/intersection (get-point-set segment1) (get-point-set segment2))))

(defn contains-intersection? [pt spoints]
  (seq (clojure.set/intersection (get-point-set spoints) #{pt})))

(defn get-intersections [wire spoints]
  (let [origin [0 0]]
    (first (reduce (fn [[res a0] a]
                     (let [points (to-point-sequence a0 a)
                           dest (:dest points)
                           intersections (cross? points spoints)]
                       (if (seq intersections)
                         [(conj res intersections) dest]
                         [res dest])))
                   [nil origin ]
                   wire))))

(defn wire-segments [wire]
  (first (reduce (fn [[res o] s]
                   (let [p (to-point-sequence o s)]
                     [(conj res p) (:dest p)]))
                 [[] [0 0]]
                 wire)))


(defn wire-distance [wire pt]
  (let [l (take-while+ (complement (partial contains-intersection? pt)) (wire-segments wire))
        fin (last l)]
    (- (reduce + (map #(Integer. (:magnitude %)) l)) (manhattan-distance (:dest fin) pt))))
...
(defn -main
  ...
  (let [inputs (parse input)
        wire1 (first inputs)
        wire2 (second inputs)
        targets (map vec (partition 2 (flatten (remove nil?( map (partial get-intersections wire1) (wire-segments wire2))))))
        wire1-distance (map (partial wire-distance wire1) targets)
        wire2-distance (map (partial wire-distance wire2) targets)]
    (reduce min (map #(reduce + %) (map vector wire1-distance wire2-distance)))))
